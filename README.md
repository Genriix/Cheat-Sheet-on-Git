...
Теперь мы можем смело отправлять свои репозитории в сеть, не боясь что их могут украсть злоумышленники.
...
> Вместо master может быть main, в зависимости какая у вас рабочая ветка, она написана в консоли после пути папки

Кстати сказать, что ```-u origin master``` стоит писать только единожды, когда мы только связываем репозитории.
Это нужно чтобы показать, какие веткы мы связываем, далее - нам это ни к чему, можем пользоваться просто командой:

```BASH
git push
```

Эта команда подойдёт если мы добавляем коммиты последовательно в одну ветку, если мы задумали добавить вторую ветку, то нужно указать это.
...
Удобно редактировать файл README прямо в среде разработки, я использую MS VS, там есть хорошие инструменты, подсведка отдельных типов текста.
Хорошая шпоргалка по Markdown (написанию README-файлов) находится [по этой ссылке](https://gist.github.com/fomvasss/8dd8cd7f88c67a4e3727f9d39224a84c#emphasis).

---

## Статус файлов в Git

У файла в гит есть 4 состояния:

1. Не отслеживается, нужно добавить к отслеживанию
2. Отслеживается, готов к коммиту
3. Актуален, идентичен к настоящему коммиту
4. Отредактирован, нужно добавить к отслеживанию
 
Что значит "нужно добавить к отслеживания"?
То, что изменения или файл видны, но если сделать коммит, то эти изменения в него не войдут, так что надо добавить их к отслеживанию командой
